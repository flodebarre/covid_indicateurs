dlData <- FALSE
source("usefulFunctions.R") # Sliding average function
...
# Load population size used by SPF
...
minDate <- as.Date("2021-12-15")
maxDate <- max(dat$date)
ylm <- c(0.5, 3)
lang <- "FR"

fname <- "pics/ratiosFrance.png"
...
polygon(x = as.Date(c(limPlot[1], limPlot[2], limPlot[2], limPlot[1], limPlot[1]), origin = "1970-01-01"), 
        y = c(1, 1, ylm[1], ylm[1], 1), border = NA, col = adjustcolor(colDecroiss, op))
# Croissance
polygon(x = as.Date(c(limPlot[1], limPlot[2], limPlot[2], limPlot[1], limPlot[1]), origin = "1970-01-01"), 
        y = c(1, 1, ylm[2], ylm[2], 1), border = NA, col = adjustcolor(colCroiss, op))
