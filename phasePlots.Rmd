...
#  plot(0)
  plotRatio(as.Date(tmp$jour), tmp$P, ylim = c(0.35, 3), axesLabels = FALSE, cex = 0.6, lightAxes = TRUE, colScale = palH, showGraduations = FALSE)
...
emptyplot()
text(0, 0, paste0("Ratios des cas d'une semaine à l'autre, ", as.Date(minDate) + 7, " -- ", max(datDep$jour)), adj = 
c(0.5, 0.5), cex = 1.5)
...
tmpDep <- datDep[which(datDep$jour >= as.Date(minDate) - 14 & datDep$cl_age90 == agCl), ]
```
...
plotPhaseDep0 <- function(idate){
  ...
  layout(matrix(1:2, ncol = 2), widths = c(5, 1.5))
  
  par(mar = c(6, 4, 2, 0), las = 1, mgp = c(2.5, 0.5, 0), tck = -0.01, bg = gray(1))
  # Initialize plot
  plot(1, type = "n", xlim = c(100, 5000), ylim = c(0.4, 4), log = "xy", 
       xlab = "7-day incidence per 100'000 inhabitants", ylab = "week-to-week ratio of 7-day incidence")
  abline(h = 1, lwd = 2)
  limPlot <- par("usr")
...
"), adj = 0, cex = 0.5, family = "mono", line = 5.5, col = gray(0.4))
  ...
  for(i in geo.departements$code_departement[nchar(geo.departements$code_departement) < 3]){
    ...
    #lines(incid7[inna], ratios7[inna], col = "red", type = "l")
    lines(incid7[inna], ratioIncid7[inna], col = colReg[reg], type = "l")
    
    points(x = incid7[nend], y = ratioIncid7[nend], col = colReg[reg], cex = 2, pch = 16)
...
system(paste0("ffmpeg -pattern_type glob -i 'pics/phaseDep_", "*", ".png' -vcodec libx264 -s 800x450 -pix_fmt yuv420p -filter:v 'setpts=8.0*PTS' pics/mapPhasePlotDep.mp4"))
