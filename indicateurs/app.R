...
server <- function(input, output) {
    ...
    # Load sliding average function
    tmp <- tempfile()
    download.file("https://raw.githubusercontent.com/flodebarre/covid_indicateurs/main/usefulFunctions.R", tmp, "auto")
    source(tmp)
    ...
    tmp <- tempfile()
    download.file("https://raw.githubusercontent.com/flodebarre/covid_indicateurs/main/data/jours_feries_metropole.csv", tmp, "auto")
    feries <- read.csv(tmp)
...
#    mtext("date", side = 1, line = 0.5)
    ...
    axis(1, at = c(mnths, mnths + 14) , labels = format(c(mnths, mnths + 14), format = dateFormat), las = 1, cex.axis = cexx, tck = tckk, lwd = 0, lwd.ticks = 1, pos = ylm[1], adj = adj)

    axis(1, at = as.Date(c("2020-06-01", "2021-01-01", "2022-01-01")), labels = c("2020", "2021", "2022"), las = 1, cex.axis = cexx, tck = tckk, lwd = 0, line = 0, adj = adj)
