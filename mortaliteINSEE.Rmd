...
plotDC <- function(codeLoc, gradMax = 60, localLevel = "region", mar = c(1, 1, 2.5, 1), cex.main = 1.5, factor.lwd = 1, cex.month = 0.7){
  ...
  for(i in seq_along(monthLab)){
    text(x = r * cos(frac2angle(thetas[i])), y = r * sin(frac2angle(thetas[i])), labels = monthLab[i], srt = 90 + 90 - thetas[i]*360 + orientation[i] * 180, col = gray(1), cex = cex.month, family = "sans")
  }
...
fname <- "pics/DCdepartements.png"
fac.dep <- 1
png(filename = fname, width = fac.dep * sum(widths.deps), height = fac.dep * sum(heights.deps), res = 300, units = "in")
