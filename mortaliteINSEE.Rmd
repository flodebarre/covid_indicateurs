...
(This is not used in the end, but kept for legacy)
...
plotRegionDC <- function(codereg, gradMax = 60){
  # codereg: geographic numerical code of the region
  ...
  for(i in 1:12){
    ...
    polygon(x = c(0, arc$x, 0), 
            y = c(0, arc$y, 0), border = gray(0.95), col = gray(0.9), lwd = 0.5)
  }
  ...
  for(i in seq_along(monthLab)){
    text(x = r * cos(frac2angle(thetas[i])), y = r * sin(frac2angle(thetas[i])), labels = monthLab[i], srt = 90 + 90 - thetas[i]*360 + orientation[i] * 180, col = gray(1), cex = 0.7, family = "sans")
  }
  ...
  for(i in seq(10, gradMax, by = 10)){
    ...
    lines(cxy$x, cxy$y, type = "l", col = gray(1), lwd = lwd.grad)
  }
  ...
  segments(x0 = head(pxy$x, -1), 
       ...
       pch = 16, col = agg$col, lwd = 1.5)
  
  title(main = reg, cex.main = 1.5, font = 1)
} # end plot function
...
```{r DCRegDROM}
fname <- "pics/INSEE_DC_DROM.png"

ech <- c(120, 120, 60, 60, NA, 60) # Scale for max graduation

png(fname, width = fac*3, height = fac*(3+2*hh), units = "in", res = 300)
...
for(codereg in regDROM){
  par(pty = "s", xpd = FALSE) # Square aspect ratios
  par(mar = c(0, 0, 1.5, 0) + 1)
  plotRegionDC(codereg, gradMax = ech[codereg])
#  plot(0, axes = FALSE, xlab = "", ylab = "")
#  title(dicReg[as.character(codereg)], cex.main = 1.5, font.main = 1)
}
