...
palTi <- met.brewer("Tam", n = length(dicoTi), "continuous", direction = 1)
names(palTi) <- dicoTi
```
...
plotIncidence <- function(tmp2){
  ...
  mf_legend(type = "typo", pos = "bottomleft", val = rev(names(dicoTi)), pal = rev(palTi), title = "Weekly indicence \nper 100'000")
  ...
  mf_title(paste0("Weekly incidence, ", dt, suffix), bg = "white", fg = "black")
}

fnI <- paste0("pics/mapEPCI_incidence_", thedate, ".png")
#mf_export(tmp2, file = fnI, width = 600, height = 600, res = 100)
layout(matrix(1:2, ncol = 1), heights = c(1.5, 4))

# France incidence

subFra <- datFra[datFra$date <= thedate, ]
par(mar = c(2, 4, 1, 1), mgp = c(2., 0.5, 0), tck = -0.02)
...
for(i in seq_along(dates)){
  ...
  mf_export(tmp2, file = fnI, width = 600, height = 700, res = 100)
  
  layout(matrix(1:2, ncol = 1), heights = c(1.5, 4))

  #---
  subFra <- datFra[datFra$date <= thedate, ]
  par(mar = c(2, 4, 1, 1), mgp = c(1.5, 0.25, 0), tck = -0.025)
  months <- seq(as.Date("2020-01-01"), Sys.Date(), by = "month")
  plot(subFra$date, subFra$Ti, col = palTi[subFra$TiCatNum], pch = 16, axes = FALSE, 
       xlab = "", ylab = "", xlim = c(min(dates), max(dates)))
...
system(paste0("ffmpeg -pattern_type glob -i 'pics/mapEPCI_incidence_", "*", ".png' -vcodec libx264 -s 600x700 -pix_fmt yuv420p -filter:v 'setpts=2*PTS' pics/mapEPCI_incidence.mp4"))
```
