...
for(i in graduations1){
  ...
  lines(cxy$x, cxy$y, type = "l", col = "gray")
}
for(i in graduations2){
  cxy <- polar2xy(i, seq(0, 1, length.out = 100))
  lines(cxy$x, cxy$y, type = "l", col = "gray", lty = 3)
}
for(i in graduations1){
...
thetas <- (1:12)/12 - 0.5/12
r <- 40000
for(i in seq_along(months)){
  text(x = r * cos(frac2angle(thetas[i])), y = r * sin(frac2angle(thetas[i])), labels = months[i], srt = 90 - thetas[i]*360)
}
...
```{r}
plot(as.Date(dat.France$date), dat.France$incid_hosp)
lines(as.Date(dat.France$date), dat.France$incid_hosp_ave7, col = "red")
range(dat.France$incid_hosp, na.rm = TRUE)
