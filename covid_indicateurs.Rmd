...
library("MetBrewer")
pal <- met.brewer("Derain", n = 3, type = "discrete")
col2020 <- pal[3]
col2021 <- pal[1]
col2022 <- pal[2]
...
addLegendPlot <- function(iend, graduations1, graduations2, r, dfinal, valfinal){
  ...
  text(x = 0, y = r, adj = c(0, 1), labels = " 2022", col = col2022)
...
Mise à jour : ", today, " | ", "@flodebarre")
}else{
  txt <- paste0("Data: 
https://www.data.gouv.fr/fr/datasets/synthese-des-indicateurs-de-suivi-de-lepidemie-covid-19/", "
Code: https://github.com/flodebarre/covid_indicateurs/blob/main/covid_indicateurs.Rmd
Date: ", today, " | ", "@flodebarre")
}
  mtext(side = 1, text = txt, cex = 0.55, line = 2, family = "mono", adj = 0)
...
if(doGif){
  ...
  system("ffmpeg -pattern_type glob -i 'pics/hosp*.png' -vcodec libx264 -s 750x700 -pix_fmt yuv420p pics/hosp.mp4")
...
for (ac in names(ageClasses)){
  ...
  cd <- uk.admis[which(is.element(uk.admis$age, ageClasses[[ac]])),]
  agg.deaths <- aggregate(cd$deaths, by = list(date = cd$date), FUN = sum)
  names(agg.deaths)[2] <- "deaths"
