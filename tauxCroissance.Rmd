...
rm(list = ls()) # I don't care, I code the way I want
```
...
plotRatio(89, "2021-06-01", dateMax)
```
...
par(bg = gray(0.65), 
    fg = gray(1), 
    col.axis = fgg, col.lab = fgg, col.ticks = fgg)
par(las = 1)
...
axis.Date(1, x = c(subdat$jour, "2021-12-15"), lwd = 0, lwd.ticks = 1)
...
text(x = rep(0.2, length(yy)), yy, labels = vals, adj = 0)


#-------------------------------
dxx <- 1
plot(rep(0, 7), rev(1:7), pch = getPCHs(1:7), bg = "white", fg = NA, xlim = c(-0.2, 1), 
     ylim = c(1 - dxx, 7 + dxx), 
     cex = getCexs(1:7), 
