...
plot(as.Date(subdat$jour), subdat$logratios7, col = colJ[as.character(subdat$weekday)], pch = 16, 
     ylim = 1.1*c(-xr, xr))

colCroiss <- "#FF0000"
colDecroiss <- "#00A5A5"

xrr <- max(abs(subdat$ratios7, na.rm = TRUE))
par(mgp = c(2.5, 0.5, 0.5), las = 1, 
    mar = c(5, 4, 5, 6))
ylm <- 1.1*exp(c(-xr, xr))
plot(as.Date(subdat$jour), subdat$ratios7, ylim = ylm, log = "y", axes = FALSE, 
     xlab = "", ylab = "Ratio du nombre de cas d'une semaine à l'autre, échelle log", 
     type = "n")
...
text(as.Date(limPlot[1], origin = "1970-01-01"), ylm[1], adj = c(0, -1), labels = "   Le nombre de cas décroît", col = colDecroiss, font = 2)
text(as.Date(limPlot[1], origin = "1970-01-01"), ylm[2], adj = c(0, 2), labels = "   Le nombre de cas croît", col = colCroiss, font = 2)
...
axis(1, at = seq(as.Date(dateMin), as.Date(dateMax)+14, by = "month"), labels = seq(as.Date(dateMin), as.Date(dateMax)+14, by = "month"), las = 3, cex.axis = cexx, tck = tckk, lwd = 0, lwd.ticks = 1, pos = ylm[1])

axis(1, at = seq(as.Date(dateMin)+16, as.Date(dateMax)+14, by = "month"), labels = seq(as.Date(dateMin)+16, as.Date(dateMax)+14, by = "month"), las = 3, cex.axis = cexx, tck = tckk, lwd = 0, lwd.ticks = 1, pos = ylm[1])
...
cexleg <- 0.85
dj <- 2
text(x = as.Date(limPlot[2], origin = "1970-01-01") + dj, y = exp(t4), labels = "x2 toutes les \n4 semaines", adj = c(0, 0.5), cex = cexleg)

text(x = as.Date(limPlot[2], origin = "1970-01-01") + dj, y = exp(t2), labels = "x2 toutes les \n2 semaines", adj = c(0, 0.5), cex = cexleg)

text(x = as.Date(limPlot[2], origin = "1970-01-01") + dj, y = exp(t1), labels = "x2 toutes les \nsemaines", adj = c(0, 0.5), cex = cexleg)

text(x = as.Date(limPlot[2], origin = "1970-01-01") + dj, y = exp(t05), labels = "x2 tous les \n5 jours", adj = c(0, 0.5), cex = cexleg)

text(x = as.Date(limPlot[2], origin = "1970-01-01") + dj, y = exp(-t4), labels = "÷2 toutes les \n4 semaines", adj = c(0, 0.5), cex = cexleg)

text(x = as.Date(limPlot[2], origin = "1970-01-01") + dj, y = exp(-t2), labels = "÷2 toutes les \n2 semaines", adj = c(0, 0.5), cex = cexleg)

text(x = as.Date(limPlot[2], origin = "1970-01-01") + dj, y = exp(-t1), labels = "÷2 toutes les \nsemaines", adj = c(0, 0.5), cex = cexleg)
...
mtext("Inspiré des @BristOliver pour le cas anglais
Données https://www.data.gouv.fr/fr/datasets/donnees-relatives-aux-resultats-des-tests-virologiques-covid-19/#
(en date de prélèvement)", side = 1, line = 3, adj = 0, cex = 0.5, family = "mono")
```
